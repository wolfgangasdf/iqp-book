...
:tags: [remove-input]
:label: b-planck
:caption: Optical spectra of blackbody radiators for different temperatures, comparing the non-quantum Rayleigh-Jeans model to the Planck model.
...
:tags: [remove-input]
:label: b-wave
:caption: An electromagnetic wave moving in $x$-direction as a function of $x$-position and time $t$. The wave is monochromatic with wavelength $\lambda=1$ µm, which corresponds in free-space to a frequency of $f=c/\lambda=3\cdot 10^{14}\, \mathrm{Hz}$ which is 300 Tera Hertz.
...
:tags: [remove-input]
:label: b-ds
:caption: In the double-slit experiment, a coherent wave is incident from the left, excites elementary waves at both slits which then interfere constructively and destructively on the screen on the right - forming an interference pattern.
...
:tags: [remove-input]
:label: b-dssp
:caption: If we use a single-photon sensitive camera, we can see the build-up of the interference pattern - but before being detected at a particular position, each photon's wavepacket must have passed both slits, otherwise the interference pattern would not be visible.
...
:tags: [remove-input]
:label: h-lines
:caption: Hydrogen emission lines. The fact that they are lines in the first place is a very strong indication that electrons have wave-like properties.
...
:tags: [remove-input]
:label: b-bohr
:caption: If electrons are waves around the core of an atom, only particular wavelengths lead to closed orbits - with this simple model called the Bohr model, a surprisingly large amount of effects can be explained - including the atomic emission lines in the previous figure!
