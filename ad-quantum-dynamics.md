...
We now assume that we are in a particular state, this means that we know $c_i(t=0)$. Now we add a time-dependent perturbation $\hat{H}^{\prime}(t)$, and we plug the state in the time-dependent Schrödinger equation.
...
For the RHS of the Schrödinger equation, differentiate $\Psi(t)$:
...
Equate the LHS and RHS and cancel the $E_n$ terms to get:
...
Now project onto a particular basis state $\psi_m$ (use orthonormality $\langle\psi_m|\psi_n\rangle=\delta_{mn}$):
...
where we additionally assume that $H_{mm}'=H_{nn}'=0$, which is reasonable because we apply a time-dependent drive to induce transitions between stationary states, not redefine them (as was done in time-independent perturbation theory).

Multiply both sides by $e^{iE_m t/\hbar}$:
...
\qquad \omega_{nm}\equiv\frac{E_n-E_m}{\hbar}
$$

Thus, we have obtained the time-derivatives of the coefficients (where we assume $E_b\gt E_a$)
...
Imagine a qubit which can be in its ground state $|a\rangle$ or excited state $|b\rangle$. We shine light (or apply a microwave field) at a frequency $\omega$ that is close to the natural transition frequency $\omega_0$ between the two states.

Instead of staying fixed in one state, the system now *coherently* oscillates between the two states. This oscillation is what we call Rabi oscillations. The probability of being in the excited state $|b\rangle$ goes up and down in time, and the rate of this oscillation is set by the Rabi frequency $\omega_r$.
:::
